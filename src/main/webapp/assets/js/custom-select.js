(function(angular, undefined) {
	...
	module.value('customSelectDefaults', {
		...
		emptyListText : 'Sem items para mostrar',
							...
							function($parse, $compile, $timeout, $q,
								...
								return {
									...
									link : function(scope, elem, attrs,
										...
										var itemTemplate = elem.html().trim()
												|| '{{'
												+ (match[2] || match[1]) + '}}',
										...
										// Create child scope for input and
										// dropdown
										var childScope = scope.$new(true);
										configChildScope();

										// Click event handler to set initial
										// values and focus when the dropdown is
										// shown
										anchorElement.on('click', function(
											...
											// If filter is not async, perform
											// search in case model changed
											// if (!options.async) {
											// getMatches();
											// }
										});
										...
										if (dependsOn) {
											scope.$watch(dependsOn, function(
													newVal, oldVal) {
												if (newVal !== oldVal) {
													childScope.matches = [];
													childScope
										...
										// Event handler for key press (when the
										// user types a character while focus is
										...
										// Event handler for Esc, Enter, Tab and
										// Down keys on input search
										...
										// Event handler for Up and Down keys on
										// dropdown menu
										ulElement
												.on(
														'keydown',
										...
										// When model changes outside of the
										// control, update the display text
										controller.$render = function() {
											setDisplayText();
										};

										// Watch for changes in the default
										// display text
										childScope.$watch(getDisplayText,
												setDisplayText);
										...
										function getOptions() {
											return angular
													.extend(
															...
															baseOptions,
															scope
																	.$eval(attrs.customSelectOptions));
										}
										...
										// when the user presses Enter inside
										// the search input
										function selectFromInput() {
											var opts = ulElement.find('li > a');
											if (opts.length > 0) {
												var ngRepeatItem = opts.eq(0)
														.scope();
										...
										function getMatches(searchTerm) {
											if (searchTerm === undefined) {
												searchTerm = (childScope.searchTerm || "")
														.trim();
											}
											var locals = {
